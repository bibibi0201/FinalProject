...
class PostureMonitorService {
  ...
  void startMonitoring(String deviceName) {
    ...
    _historyRef.onValue.listen((event) async {
      ...
      /// ถ้าเป็น record เดิมไม่ต้องทำซ้ำ
      if (latestTime == _lastAlertTime) return;

      _lastAlertTime = latestTime;
      ...
      if (_badCount >= 3) {
        ...
        /// reset counter เพื่อให้แจ้งเตือนรอบต่อไปได้
        _badCount = 0;
      }
    });
  }
  ...
  Future<void> _showNotification(String title, String body) async {
    ...
    /// ถ้าปิด notification → ไม่ต้องแจ้งเตือน
    if (!enabled) return;
